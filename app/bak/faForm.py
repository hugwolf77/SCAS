...
class seekerEFA(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        # select data file
        self.path_btn.clicked.connect(self.file_exeplore)
        self.path_edit.returnPressed.connect(self.Input_path)   
    ...
    def file_exeplore(self):
        ...
        self.path_edit.setText(self.select_data)
        self.path_print.setText(self.select_data)

    @QtCore.Slot()
    def Input_path(self):
        edit_path = self.path_edit.text()
        print(f"Enter Path: {edit_path}")
        self.path_edit.setText(edit_path)

    @QtCore.Slot()
    def Load_data(self):
        self.path_edit.setText(self.select_data)
        self.path_print.setText(self.select_data)
        headerRow = self.header_row.value()
        if self.indexCol_edit.text() == '' : indexCol = None 
        else: indexCol = self.indexCol_edit.text()
        ...
        self.DataViewTable.setColumnCount(colCount)
        self.DataViewTable.setHorizontalHeaderLabels(self.rawData.columns)
        self.DataViewTable.setRowCount(rowCount)

        # view widget table data setting
        for r in range(rowCount):
            for c in range(colCount):
                self.DataViewTable.setItem(r,c, QTableWidgetItem(str(self.rawData.iloc[r,c])))
